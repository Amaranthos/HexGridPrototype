	...
	public void CheckConditions(){
		if(Logic.Inst.gamePhase == GamePhase.PlacingPhase){
			...
			if(Logic.Inst.CurrentPlayerNum == 0)
			{
				...
				if(Logic.Inst.SelectedUnit != null)
				{
					// Does Player own the unit?
					if(Logic.Inst.SelectedUnit.Owner == Logic.Inst.CurrentPlayer)
					{
						// Hero Selected?
					...
					// Player does not own the Unit
					else// if(Logic.Inst.SelectedUnit.Owner != Logic.Inst.CurrentPlayer)
					{
						P1HeroShrine = false;
						P1Unit = true;
						P1Hero = false;
